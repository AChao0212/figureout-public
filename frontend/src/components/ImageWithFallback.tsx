...
export default function ImageWithFallback({ src, alt = "", className = "", compact = false }: Props) {
  ...
  if (!src || failed) {
    return (
      <div
        className={`flex flex-col items-center justify-center gap-1 bg-[#161b22] text-[#484f58] ${className}`}
        role="img"
        aria-label={alt || "無法載入圖片"}
        title="圖片來源 (hpoi.net) 暫時無法載入"
      >
        <svg viewBox="0 0 24 24" className="h-6 w-6 fill-none stroke-current" strokeWidth="1.5" aria-hidden="true">
          <rect x="3" y="5" width="18" height="14" rx="2" />
          <circle cx="9" cy="10" r="1.5" />
          <path d="M21 17l-5-5-8 8" />
        </svg>
        {!compact && <span className="text-[9px]">圖片暫時無法載入</span>}
      </div>
    );
  }
